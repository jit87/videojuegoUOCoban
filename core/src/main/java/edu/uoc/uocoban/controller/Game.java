...
public class Game {
    ...
    public boolean nextLevel() throws LevelException {
        ...
        if(isFinished()){
            ...
            //Checks if there is a level
            if (level != null)
                //The game score must be updated when a level is finished
                setScore(this.getScore() + this.getRemainingMovements());
                //Loading and updating the level
    ...
    public boolean isLevelCompleted() {
        ...
        if (level != null) {
            for (MapItem mapItem : level.getMapItemList()) {
                if (mapItem instanceof Destination) {
                    Destination destination = (Destination) mapItem;
                    if (destination.getBox() == null || !destination.getBox().isDelivered()) {
                        return false;
                    }
                }
            }
    ...
    public boolean hasLostLevel() {
        if(getRemainingMovements()==0 && !level.hasWon()){
            return true;
        } else
            return false;
    }
